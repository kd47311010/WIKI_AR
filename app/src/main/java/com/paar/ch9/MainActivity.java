...
public class MainActivity extends AugmentedActivity {
    ...
    private static boolean download(NetworkDataSource source, com.paar.ch9.model.Query query) {
        if (source == null) return false;

        List<Marker> markers = null;
        try {
            markers = source.parse(query);
                    ...
                    new Runnable() {
                        ...
                        public void run() {
                            for (NetworkDataSource source : sources.values())
                                download(source, new Query(lat, lon, alt, ARData.getRadius(), "ko"));
                        }
                    }
            );
